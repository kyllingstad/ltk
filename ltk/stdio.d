...
/** Struct for buffered I/O. */
struct BufferedFile
{
private:
    enum size_t bufferSize = 4096;

    struct Impl
    {
        UnbufferedFile ufile;
        ubyte[] buffer;
        ubyte[] bufferView;
        int refs;
    }
...
public:
    ...
    {
        p = new Impl;
        p.ufile = UnbufferedFile(filename, mode, false);
        p.buffer = new ubyte[bufferSize];
        p.refs = (autoClose ? 1 : 999);
    }
    ...
    /** Attempt to read a single value from the file. */
    bool read(T)(ref T t) if (!isArray!T)
    {
        ensureBuffer();
        if (p.bufferView.length == 0) return false;

        t = p.bufferView[0];
        p.bufferView = p.bufferView[1 .. $];
        return true;
    }
    ...
    void ensureBuffer()
    {
        if (p.bufferView.length > 0) return;
        p.bufferView = p.ufile.read(p.buffer);
    }
